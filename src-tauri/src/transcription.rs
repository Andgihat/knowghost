...
pub struct TranscriptionRequest {
    ...
    /// OpenAI-совместимый базовый URL (без /v1). Если не задан — используется api.openai.com.
    #[serde(default)]
...
async fn transcribe_openai(request: TranscriptionRequest) -> Result<TranscriptionResponse> {
    ...
    // Настраиваемый базовый URL: /v1/audio/transcriptions
    let base = request
        .api_base_url
        .as_deref()
    ...
    if !status.is_success() {
        ...
        return Err(anyhow!("API error ({}): {} - {}", base, status, error_text));
    }
...
async fn transcribe_local(request: TranscriptionRequest) -> Result<TranscriptionResponse> {
    ...
    // whisper.cpp / Fast-Whisper могут вернуть JSON {"text": ...} либо чистый текст.
    ...
    if text.to_lowercase().contains("transcribe verbatim") {
