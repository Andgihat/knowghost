...
/// Сохранённый API-ключ, привязанный к провайдеру.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SavedApiKey {
    /// Уникальный идентификатор (UUID).
    pub id: String,
    /// Провайдер: "openai", "deepseek", "google", "openrouter", "custom"
    pub provider: String,
    /// Отображаемое имя (пользователь задаёт).
    pub name: String,
    /// Сам ключ.
    pub api_key: String,
    /// Базовый URL для OpenAI-совместимых провайдеров (без /v1).
    #[serde(default)]
...
pub struct AppConfig {
    ...
    /// Список сохранённых API-ключей.
    #[serde(default)]
    pub saved_api_keys: Vec<SavedApiKey>,
    /// ID активного ключа из saved_api_keys.
    #[serde(default)]
    pub active_api_key_id: Option<String>,
    #[serde(default)]
    pub whisper_server_exe: Option<String>,
    #[serde(default)]
    pub whisper_server_model: Option<String>,
    #[serde(default)]
    pub stt_source: Option<String>,
    #[serde(default)]
    pub screen_separate: Option<bool>,
    #[serde(default)]
    pub screen_base_url: Option<String>,
    #[serde(default)]
    pub screen_api_key: Option<String>,
    #[serde(default)]
    ...
    #[serde(default)]
    pub use_default_llm_prompt: bool,
    #[serde(default)]
    pub use_default_transcription_prompt: bool,
...
impl Default for AppConfig {
    fn default() -> Self {
        let mut cfg = Self {
            ...
            whisper_server_exe: None,
            whisper_server_model: None,
            stt_source: None,
            screen_separate: None,
            ...
            use_default_llm_prompt: false,
            use_default_transcription_prompt: true,
