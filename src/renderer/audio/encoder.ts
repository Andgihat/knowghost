...
// Целевая частота для распознавания: whisper.cpp принимает только 16 кГц (не ресемплит).
export const STT_TARGET_RATE = 16000;

// Даунмикс в моно + линейный ресемпл до 16 кГц. Убирает зависимость от ffmpeg (--convert)
// и подходит всем STT-бэкендам (whisper.cpp / OpenAI / Google).
export function toMono16k(channels: Float32Array[], inRate: number): Float32Array {
    const numChannels = Math.max(1, channels.length);
    ...
    for (let ch = 0; ch < numChannels; ch++) {
        ...
        for (let i = 0; i < frames; i++) {
            mono[i] += (c[i] || 0) / numChannels;
        }
    }
    const rate = Math.max(8000, Math.floor(inRate || STT_TARGET_RATE));
    if (rate === STT_TARGET_RATE || frames === 0) {
        return mono;
    }
    const ratio = STT_TARGET_RATE / rate;
