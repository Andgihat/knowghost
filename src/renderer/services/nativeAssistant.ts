...
const APP_NAME = 'KnowGhost';
const LLM_PROMPT_DEFAULT_KEY = 'ai.llmPromptDefault';

function resolveSystemPrompt(settings: AppSettings): string {
    if (settings.useDefaultLlmPrompt) return t('ai.llmPromptDefault');
    if (settings.llmPrompt?.trim()) return settings.llmPrompt.trim();
    return t('ai.llmPromptDefault');
}
...
// Runtime-configurable OpenAI-compatible endpoint (cloud OpenAI/DeepSeek/OpenRouter or local llama.cpp/TabbyAPI).
// Enter the base WITHOUT the /v1 suffix; `/v1/chat/completions` is appended below.
const resolveOpenAiBase = (settings: AppSettings): string =>
    settings.apiBaseUrl?.trim().replace(/\/$/, '') || OPENAI_BASE;
const SCREEN_OPENAI_MODEL = 'gpt-4o-mini';
const SCREEN_GEMINI_MODEL = 'gemini-1.5-flash';
const MAX_HISTORY_MESSAGES = 40;

// Vision (обработка экрана): при screenSeparate=true использует СВОЙ эндпоинт/ключ/модель
// (напр. DeepSeek для текста + OpenRouter для vision). Иначе — те же, что у текстового LLM.
const resolveScreenBase = (settings: AppSettings): string => {
    if (settings.screenSeparate) {
        // Отдельный vision-провайдер независим: пустой base = OpenAI по умолчанию.
        return settings.screenBaseUrl?.trim().replace(/\/$/, '') || OPENAI_BASE;
    }
    return resolveOpenAiBase(settings);
};
const resolveScreenModel = (settings: AppSettings): string => {
    if (settings.screenSeparate) {
        return settings.screenModel?.trim() || SCREEN_OPENAI_MODEL;
    }
    // Не отдельно → та же модель, что у текстового LLM.
    return settings.apiLlmModel?.trim() || settings.llmModel?.trim() || SCREEN_OPENAI_MODEL;
};
const resolveScreenKey = (settings: AppSettings): string => {
    const key = (settings.screenSeparate && settings.screenApiKey?.trim())
        ? settings.screenApiKey.trim()
        : settings.openaiApiKey?.trim();
    if (!key) {
        throw new Error('Provide a vision API key (screen processing) or an OpenAI key');
    }
    return key;
};
...
const buildTranscriptionPrompt = (settings: AppSettings): string | undefined => {
    ...
    if (settings.useDefaultTranscriptionPrompt) {
        return `${t('ai.transcriptionPromptDefault')}\n\n${guard}`;
    }
    if (userPrompt) {
        return `${userPrompt}\n\n${guard}`;
    }
...
async function transcribeWithOpenAi(
    ...
    logRequest('transcribe:openai', 'start', {model: resolvedModel, mime, apiBase: settings.apiBaseUrl || 'default'});
    ...
    try {
        const result = await invoke<{ text: string }>('transcribe_audio', {
            request: {
                ...
                api_base_url: settings.apiBaseUrl?.trim() || undefined,
            },
        });
    ...
    try {
        const result = await invoke<{ text: string }>('transcribe_audio', {
            request: {
                ...
                mime_type: mime || 'audio/wav',
                filename,
                prompt: undefined, // Don't send prompt to whisper
            },
        });
        ...
        if (text) {
            ...
            if (isPromptText) {
                logRequest('transcribe:local', 'error', {
                    message: 'whisper returned prompt text instead of transcription',
                    receivedText: text,
                    bufferSize: buffer.byteLength,
                });
                throw new Error('whisper returned prompt text instead of transcription. The audio file may be empty, too short, or contain no speech. Please check that audio is being captured correctly.');
            }
        }

        if (!text || text.trim().length === 0) {
            logRequest('transcribe:local', 'error', {
                message: 'Empty transcription from whisper',
                bufferSize: buffer.byteLength,
            });
            throw new Error('whisper returned empty transcription. The audio file may be empty, too short, or contain no speech.');
        }
...
async function chatCompletion(
    ...
    const apiKey = ensureOpenAiKey(settings);
    const url = `${resolveOpenAiBase(settings)}/v1/chat/completions`;
...
const buildGeminiBody = (
    ...
    if (settings.llmPrompt?.trim()) {
        body.systemInstruction = {
            role: 'system',
            parts: [{text: resolveSystemPrompt(settings)}],
        };
    }
    return body;
};
...
async function streamChatCompletion(
    ...
    // Build user message — plain text or multimodal
...
const SUMMARY_SYSTEM_PROMPT = `Ты — ассистент, который делает подробные структурированные конспекты диалогов (вопрос–ответ: собеседования, консультации, разбор задач).

Тебе передают диалог. Преврати его в конспект на русском языке в формате Markdown. Это именно конспект для последующего повторения материала, а не сверхкраткая сводка — сохраняй полезные детали.

Структура:

## Суть
Одно-два предложения: о чём был диалог в целом.

## Разбор по темам
Пройди по значимым вопросам и темам диалога. Глубину подстраивай под важность и сложность материала:
- Важное, сложное или неочевидное раскрывай подробно: суть, ключевые детали, логику/шаги решения, нюансы, подводные камни, примеры.
- Обыденное и очевидное давай коротко — одним-двумя предложениями.
Оформляй каждую тему подзаголовком \`###\` или жирным зачином, дальше — связное объяснение (можно списками, где уместно).

## Термины и понятия
Список встретившихся технических терминов с кратким пояснением. Раздел опусти, если терминов не было.

## Код
Ключевые фрагменты кода из диалога в блоках с указанием языка. Раздел опусти, если кода не было.

## Выводы
Главные выводы и практические рекомендации по итогам диалога.

Правила:
- Приоритет — полнота и польза для повторения, а не краткость ради краткости.
- Без вступлений вроде «Вот конспект» — сразу по структуре.
- Не выдумывай того, чего не было в диалоге.
- Сохраняй технические термины как есть (Redis, HTTP, async и т.п.).
- Пустой раздел пропускай целиком, не пиши «нет данных».`;

const SUMMARY_SYSTEM_PROMPT_EN = `You are an assistant that produces detailed, structured notes (a study synopsis) from dialogs (Q&A: interviews, consultations, problem walkthroughs).

You are given a dialog. Turn it into notes in Markdown. This is a synopsis for later review, not an ultra-short summary — keep useful detail.

Structure:

## Overview
One or two sentences: what the dialog was about overall.

## Breakdown by topic
Go through the significant questions and topics. Adapt depth to importance and complexity:
- Explain important, complex or non-obvious things in detail: the gist, key details, reasoning/steps, caveats, pitfalls, examples.
- Keep mundane and obvious things short — one or two sentences.
Format each topic with a \`###\` subheading or a bold lead-in, followed by a coherent explanation (lists where it fits).

## Terms and concepts
A list of technical terms that came up, each with a short explanation. Omit the section if there were none.

## Code
Key code snippets from the dialog in fenced blocks with a language tag. Omit the section if there was no code.

## Takeaways
The main conclusions and practical recommendations from the dialog.

Rules:
- Prioritize completeness and review value over brevity for its own sake.
- No preamble like "Here is the summary" — start with the structure.
- Do not invent anything that was not in the dialog.
- Keep technical terms as-is (Redis, HTTP, async, etc.).
- Skip any empty section entirely; do not write "no data".`;

export async function summarizeConversation(dialogText: string): Promise<string> {
    const settings = await loadSettings();
    const prompt = getLang() === 'en' ? SUMMARY_SYSTEM_PROMPT_EN : SUMMARY_SYSTEM_PROMPT;
    // Подменяем system-prompt на конспект-инструкцию, не трогая основной промпт коуча.
    return chatCompletion(dialogText, {...settings, llmPrompt: prompt});
}
...
export async function assistantAskChat(args: AskChatRequest) {
    ...
    runWithActiveStream(requestId, (controller) =>
        streamChatCompletion(args.text, requestId, settings, history, controller, args.imageBase64, args.imageMime)
    );
}
...
async function processScreenWithOpenAi(
    ...
    const apiKey = resolveScreenKey(settings);
    const resolvedModel = resolveScreenModel(settings);
    const url = `${resolveScreenBase(settings)}/v1/chat/completions`;
    const response = await fetchWithTimeout(
        url,
        {
            method: 'POST',
    ...
    return {answer: normalized, model: resolvedModel};
}
